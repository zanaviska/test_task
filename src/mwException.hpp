...
public:
    ...
    /*!\param errCode error code of the exception.
        ...
        provided
    */
    explicit mwException(unsigned long errCode, const std::string &errMsg = "Unknown error.",
                         const mwException *prevLevel = 0);

    //! Construct an exception object
    explicit mwException(unsigned long errCode, const char *errMsg = "Unknown error.",
                         const mwException *prevLevel = 0);
